...
if __name__ == "__main__":
    ...
    for seed in seeds:
        intersect = sorted(get_intersect(seed))

        print(len(intersect))

        # print(intersect)

        path_adv = os.getcwd() + r'/adversarial-training/result/perturb_data_only/seed'+str(seed)+"/"
        dir_list_adv = [f[:-4] for f in os.listdir(path_adv) if "ipynb" not in f]

        for exp_name in intersect:
            if exp_name in intersect and exp_name not in dir_list_adv and 'ipynb' not in exp_name and 'xlmrlarge' in exp_name and 'sentiment' in exp_name:
                # print(exp_name)
