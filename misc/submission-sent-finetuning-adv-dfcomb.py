...
def main(        
    ...
    if "sentiment" in trainpath: 
        ...
        finetuned_model, best_epoch = fine_tuning_model_es(model, i2w, train_loader, test_loader, epochs=1, patience=1)
        LABEL2INDEX = {'positive': 0, 'neutral': 1, 'negative': 2}
    ...
    temp_df_orig.to_csv(os.getcwd() + r'/submission-sent/pred.txt', index=False)
    ...
    test_df.to_csv(os.getcwd() + r'/submission-sent/'+str(exp_name)+".csv", index=False)
...
if __name__ == "__main__":
    main(
        ...
        exp_name='xlmrlarge-sentiment-codemixing-su-adv-0.6',
        seed=42
    )